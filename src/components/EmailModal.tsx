...
const EmailModal: React.FC<EmailModalProps> = ({ recipients, emailType, onClose, onSend }) => {
  ...
  const generateRegistrationNumber = (applicant: Applicant) => {
    // Generate a registration number based on applicant data
    const year = new Date().getFullYear();
    const hash = applicant.email.split('@')[0].slice(-3).toUpperCase();
    const index = Math.abs(applicant.cpf.split('').reduce((a, b) => a + b.charCodeAt(0), 0)) % 1000;
    return `SAIL-${year}-${String(index).padStart(3, '0')}`;
  };

  const generateApprovalEmail = (applicant: Applicant) => {
    const registrationNumber = generateRegistrationNumber(applicant);
...
Office of I/c – SAIL`;
  ...
  const generateShortlistedEmail = (applicant: Applicant) => {
    const registrationNumber = generateRegistrationNumber(applicant);
    const term = applicant.term === 'Summer' ? 'ग्रीष्मकालीन' : 'शीतकालीन';
    const name = applicant.name;
...
hereby cautioned that any dissemination, distribution or copying of this communication is prohibited. If you have received this transmission in error, please delete this mail and notify us immediately at admin_ongcmail@ongc.co.in`;
  ...
  const handleSendEmails = async () => {
    ...
    for (let i = 0; i < eligibleRecipients.length; i++) {
      ...
      try {
        const emailContent = emailType === 'approval' ? 
          generateApprovalEmail(recipient) : 
          generateShortlistedEmail(recipient);
        
        const subject = emailType === 'approval' ? 
          `ONGC Dehradun - Application Acknowledgment (Reg: ${generateRegistrationNumber(recipient)})` :
          `ONGC Dehradun - Training Confirmation (Reg: ${generateRegistrationNumber(recipient)})`;
            ...
            <div className="bg-gray-50 p-4 rounded-lg max-h-60 overflow-y-auto">
              <pre className="text-xs text-gray-700 whitespace-pre-wrap font-mono">
                {eligibleRecipients.length > 0 ? (
                  emailType === 'approval' ? 
                    generateApprovalEmail(eligibleRecipients[0]) : 
                    generateShortlistedEmail(eligibleRecipients[0])
                ) : (
          ...
          <div className="flex justify-end space-x-3">
            ...
            {eligibleRecipients.length > 0 && (
              ...
              >
                ...
                <span>
                  {sending ? `Sending... (${sentCount}/${eligibleRecipients.length})` : 
